...
{
    cmd_args args;
    args.register_key("--grid_type=","{string} type of the radial grid; "
            "one of linear, exponential, power, or lin_exp followed by the additional grid parameter");
    ...
    auto gt = get_radial_grid_t(grid_t);
    ...
    //    if (radial_grid.dx(0) < 1e-10) {
    //        std::cout << "small radial grid step" << std::endl;
    //    }
